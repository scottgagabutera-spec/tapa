...
export default function CarrierDashboard() {
      ...
      try {
        ...
        if (user) {
          ...
          const { data: profile } = await supabase
            .from('profiles')
            .select('name, avatar_color, id_verified, rating')
            .eq('id', user.id)
            .single();
          ...
          // Load real trips for this carrier
          ...
          // Load real booking requests for this carrier's trips
          ...
          const hasRealData = (!tripsError && rawTrips && rawTrips.length > 0) ||
                              (!bookingsError && rawBookings && rawBookings.length > 0);

          if (hasRealData) {
            // Map real trips
            if (rawTrips && rawTrips.length > 0) {
              const mappedTrips: Trip[] = rawTrips.map((t: any) => ({
                id: t.id,
                from: t.from_city,
                to: t.to_city,
            ...
            // Map real booking requests
            if (rawBookings && rawBookings.length > 0) {
              const mappedRequests: Request[] = rawBookings.map((b: any) => {
  ...
  const handleAccept = async (id: string) => {
    ...
    // Persist to Supabase if real booking (UUID)
  ...
  const displayName = userName || 'Carrier';
    ...
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '80px clamp(16px,4vw,48px) 60px' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          <h1 style={{ fontSize: 'clamp(24px,4vw,32px)', fontWeight: '800', margin: '0 0 8px', letterSpacing: '-0.5px' }}>{firstName}.</h1>
                ...
                <div key={req.id}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px', marginBottom: isPending ? '16px' : '0' }}>
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      ...
                      {req.itemDesc && <span style={{ padding: '4px 10px', background: 'rgba(139,155,180,0.1)', border: `1px solid rgba(139,155,180,0.2)`, borderRadius: '8px', fontSize: '12px', color: C.muted }}>{req.itemDesc}</span>}
                    </div>
