...
export default function TripNewPage() {
  ...
  const [capacity, setCapacity] = useState(''); const [price, setPrice] = useState('');
  const [acceptedTypes, setAcceptedTypes] = useState<string[]>([]); const [bio, setBio] = useState('');
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('trips').insert({ carrier_id: user.id, from_city: from, to_city: to, date, airline, flight_no: flightNo, capacity_kg: parseFloat(capacity), price_per_kg: parseFloat(price), item_types: acceptedTypes, status: 'active' });
      if (insertError) throw insertError;
      setSubmitted(true);
    } catch (err) { setError((err as any).message || 'Failed to publish trip'); }
    ...
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: "'Inter', sans-serif", display: 'flex', flexDirection: 'column' }}>
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, padding: '16px clamp(16px,4vw,24px)', display: 'flex', alignItems: 'center', background: 'rgba(13,27,42,0.92)', backdropFilter: 'blur(12px)', borderBottom: `1px solid ${C.border}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }} onClick={() => router.push('/')}>
          ...
          <span style={{ fontSize: '20px', fontWeight: '700', color: C.text }}>tapa</span>
        </div>
      </nav>
      <main style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '100px clamp(16px,4vw,24px) 40px' }}>
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '20px', padding: 'clamp(32px,5vw,48px) clamp(24px,5vw,32px)', maxWidth: '480px', width: '100%', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '15px', color: C.muted, lineHeight: '1.7', marginBottom: '28px' }}>Your route from <strong style={{ color: C.text }}>{from}</strong> to <strong style={{ color: C.text }}>{to}</strong> on {date} is now live.</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <button style={{ padding: '14px', background: 'transparent', border: `1px solid ${C.border}`, borderRadius: '12px', color: C.muted, fontSize: '15px', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit' }} onClick={() => router.push('/dashboard/carrier')}>My Dashboard</button>
          </div>
        </div>
      </main>
    </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: "'Inter', sans-serif" }}>
      ...
      `}</style>
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, padding: '16px clamp(16px,4vw,24px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(13,27,42,0.92)', backdropFilter: 'blur(12px)', borderBottom: `1px solid ${C.border}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }} onClick={() => router.push('/')}>
          <div style={{ width: '36px', height: '36px', background: C.coral, borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M12 3L20 20H4L12 3Z" fill="white"/></svg></div>
          <span style={{ fontSize: '20px', fontWeight: '700', color: C.text }}>tapa</span>
        </div>
      ...
      <main style={{ maxWidth: '680px', margin: '0 auto', padding: '80px clamp(16px,4vw,24px) 60px' }}>
        ...
        {step === 1 && (
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '20px', padding: 'clamp(20px,3vw,28px)' }}>
            ...
            <div style={{ fontSize: '14px', color: C.muted, marginBottom: '20px' }}>Enter your origin, destination, and flight details.</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div className="trip-row">
                <div><label style={lbl}>From</label><input type="text" placeholder="e.g. Manila" value={from} onChange={e => setFrom(e.target.value)} style={inp} onFocus={e => (e.target.style.borderColor = C.coral)} onBlur={e => (e.target.style.borderColor = C.border)}/></div>
                <div><label style={lbl}>To</label><input type="text" placeholder="e.g. Dubai" value={to} onChange={e => setTo(e.target.value)} style={inp} onFocus={e => (e.target.style.borderColor = C.coral)} onBlur={e => (e.target.style.borderColor = C.border)}/></div>
              </div>
              <div><label style={lbl}>Travel date</label><input type="date" value={date} onChange={e => setDate(e.target.value)} style={{ ...inp, colorScheme: 'dark' }} onFocus={e => (e.target.style.borderColor = C.coral)} onBlur={e => (e.target.style.borderColor = C.border)}/></div>
              <div className="trip-row">
                <div><label style={lbl}>Airline</label><select value={airline} onChange={e => setAirline(e.target.value)} style={{ ...inp, cursor: 'pointer' }}><option value="">Select airline</option>{AIRLINES.map(a => <option key={a} value={a}>{a}</option>)}</select></div>
                <div><label style={lbl}>Flight number</label><input type="text" placeholder="e.g. EK 334" value={flightNo} onChange={e => setFlightNo(e.target.value)} style={inp} onFocus={e => (e.target.style.borderColor = C.coral)} onBlur={e => (e.target.style.borderColor = C.border)}/></div>
              </div>
            </div>
            <button style={{ width: '100%', marginTop: '20px', padding: '14px', background: step1Valid ? C.coral : C.border, border: 'none', borderRadius: '12px', color: step1Valid ? C.text : C.muted, fontSize: '15px', fontWeight: '700', cursor: step1Valid ? 'pointer' : 'not-allowed', fontFamily: 'inherit' }} onClick={() => step1Valid && setStep(2)}>Continue</button>
          </div>
        )}
        ...
        {step === 2 && (
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '20px', padding: 'clamp(20px,3vw,28px)' }}>
            ...
            <div style={{ fontSize: '14px', color: C.muted, marginBottom: '20px' }}>Set how much you can carry and what you charge.</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div className="trip-row">
                <div>
                ...
                <div>
                  ...
                  <input type="number" min="1" max="100" step="0.5" placeholder="e.g. 8" value={price} onChange={e => setPrice(e.target.value)} style={inp} onFocus={e => (e.target.style.borderColor = C.coral)} onBlur={e => (e.target.style.borderColor = C.border)}/>
                  {capacity && price && <div style={{ fontSize: '13px', color: C.muted, marginTop: '6px' }}>Max: <span style={{ color: C.coral, fontWeight: '600' }}>${(parseFloat(capacity) * parseFloat(price)).toFixed(2)}</span></div>}
                </div>
              </div>
              <div>
                <label style={lbl}>Item types you will carry</label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>{ITEM_TYPES.map(t => <div key={t} style={typeChip(acceptedTypes.includes(t))} onClick={() => toggleType(t)}>{t}</div>)}</div>
              </div>
              <div>
                <label style={lbl}>About you <span style={{ fontWeight: '400' }}>(optional)</span></label>
                <textarea placeholder="Tell senders about yourself..." value={bio} onChange={e => setBio(e.target.value)} style={{ ...inp, resize: 'vertical', minHeight: '80px' }} onFocus={e => (e.target.style.borderColor = C.coral)} onBlur={e => (e.target.style.borderColor = C.border)}/>
              </div>
            </div>
        ...
        {step === 3 && (
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '20px', padding: 'clamp(20px,3vw,28px)' }}>
            ...
            <div style={{ background: C.inputBg, border: `1px solid ${C.border}`, borderRadius: '16px', padding: '20px', marginBottom: '16px' }}>
              <div style={{ fontSize: '16px', fontWeight: '700', marginBottom: '4px' }}>{from} → {to}</div>
              <div style={{ fontSize: '13px', color: C.muted, marginBottom: '16px' }}>{airline} · {flightNo} · {date}</div>
              <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                <div><div style={{ fontSize: '18px', fontWeight: '800', color: C.coral }}>${price}<span style={{ fontSize: '13px', fontWeight: '400', color: C.muted }}>/kg</span></div><div style={{ fontSize: '12px', color: C.muted }}>Price</div></div>
                <div><div style={{ fontSize: '18px', fontWeight: '700' }}>{capacity} kg</div><div style={{ fontSize: '12px', color: C.muted }}>Capacity</div></div>
              </div>
            </div>
            {[{ label: 'Accepts', value: acceptedTypes.join(', ') }].map(row => (
              <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: `1px solid ${C.border}` }}>
                ...
                <span style={{ fontSize: '14px', fontWeight: '600', textAlign: 'right', maxWidth: '240px' }}>{row.value}</span>
              </div>
            ))}
            ...
            <div style={{ display: 'flex', gap: '12px', marginTop: '20px' }}>
              ...
              <button style={{ flex: 1, padding: '14px', background: C.coral, border: 'none', borderRadius: '12px', color: C.text, fontSize: '15px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }} onClick={handlePublish}>{loading ? 'Publishing...' : 'Publish Trip'}</button>
            </div>
          </div>
        )}
      </main>
